...
class LanguageEncoderDecoder():
    ...
    def loss(self, _prevent_positional=None, labels=None, logits=None, lan1_meaning=None, lan2_meaning=None):
        ...
        # TODO: this is some bad code, it should be avoided
        paddings = tf.constant([[0, 0], [0, 1]])
        labels = tf.pad(labels[:, 1:], paddings, 'SYMMETRIC')

        target_weights = tf.where(tf.equal(labels, tf.constant(self.end_token, dtype=tf.int32)),
                                  tf.ones(labels.shape, dtype=tf.float32),
                                  tf.zeros(labels.shape, dtype=tf.float32))
        ...
        train_loss = tf.reduce_sum(cross_entropy * target_weights) + tf.reduce_mean(meaning_se)
...
def quick_test(_input, lan1, lan2):
    _embedded_input = lan1.embedder(_input=_input)

    _output, _output_state = lan1.encoder(_input=_input,
                                          _input_sequence_length=_input_sequence_length)

    _output, _ = lan2.decoder(_input_states=_output,
                              _input_sequence_length=_input_sequence_length,
                              mode='denoising',
                              _encoder_inputs=_embedded_input)
    return _output
...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(1):
            for i in range(len(lan1_sents) // hparams1.batch_size - 1):
                start_id = i * hparams1.batch_size
                stop_id = (i+1) * hparams1.batch_size
                if i > 2:
                    break

                ress = tuple(sess.run([loss_den1, loss_den2, loss_back1, loss_back2,
                                       optimizer_den1, optimizer_den2, optimizer_back1, optimizer_back2],
                                      feed_dict={_input_lan1: np.array(lan1_sents[start_id:stop_id]),
                                                 _input_lan2: np.array(lan2_sents[start_id:stop_id])}))
                print(f'ep:{epoch}: {ress[0]:.4f}, {ress[1]:.4f}, {ress[2]:.4f}, {ress[3]:.4f}')

        # a = np.array([1577, 29, 18, 26, 100, 344, 318, 1576, 7, 1322, 0, 46, 1576, 1130, 1323, 500, 4, 1576,
        #               4, 1576, 259, 1576, 319, 663, 1, 0, 1324, 79, 462, 197, 500, 1578, 1578, 1578, 1578,
        #               1578, 1578, 1578, 1578, 1578, 1578, 1578, 1578, 1578, 1578, 1578, 1578, 1578, 1578, 1578])
        #
        # in_test = tf.constant([[1577, 29, 18, 26, 100, 344, 318, 1576, 7, 1322, 0, 46, 1576, 1130, 1323, 500, 4, 1576,
        #                        4, 1576, 259, 1576, 319, 663, 1, 0, 1324, 79, 462, 197, 500, 1578, 1578, 1578, 1578,
        #                        1578, 1578, 1578, 1578, 1578, 1578, 1578, 1578, 1578, 1578, 1578, 1578, 1578, 1578, 1578]], dtype=tf.int32)
        #
        ...
        #
        # with open(f'{test_file_path}/{LANS[0]}_vocab.json', 'r') as fp:
        #     vocab_de = json.load(fp)
        #
        # with open(f'{test_file_path}/{LANS[1]}_vocab.json', 'r') as fp:
        ...
        # lan2_enc_dec = LanguageEncoderDecoder(hparams2, lan1_enc_dec)
        #
        # out = quick_test(in_test, lan1_enc_dec, lan2_enc_dec)
        #
        # out1 = sess.run(out)
        #
        # print(utils.ids2wordlist(a, vocab_en))
        # print(utils.ids2wordlist(out1, vocab_de))
