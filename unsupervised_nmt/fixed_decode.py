...
def fixed_decode(decoder,
  ...
  with variable_scope.variable_scope(scope, "decoder") as varscope:
    ...
    if num_iterations is not None:
      num_iterations = ops.convert_to_tensor(
          num_iterations, dtype=dtypes.int32, name="maximum_iterations")
      if num_iterations.get_shape().ndims != 0:
    ...
    if num_iterations is not None:
      initial_finished = math_ops.logical_or(
          initial_finished, 0 >= num_iterations)
    ...
    def condition(unused_time, unused_outputs_ta, unused_state, unused_inputs,
                  unused_finished, sequence_lengths):
      return math_ops.logical_not(math_ops.reduce_all(math_ops.equal(sequence_lengths, num_iterations)))
    ...
    def body(time, outputs_ta, state, inputs, finished, sequence_lengths):
      ...
      if num_iterations is not None:
        next_finished = math_ops.logical_or(
            next_finished, time + 1 >= num_iterations)
