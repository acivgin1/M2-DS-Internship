...
class LanguageEncoderDecoder():
    ...
    def encoder(self, _input, _input_sequence_length):
        if self.lan_encoder_decoder is None:
            ...
            with tf.variable_scope(f'{self.name}_encoder', reuse=tf.AUTO_REUSE):
                self.fw_cell = tf.nn.rnn_cell.GRUCell(num_units=self.hparams.num_units // 2, reuse=tf.AUTO_REUSE, name=f'{self.name}_fw/', dtype=self.hparams.dtype)
                self.bw_cell = tf.nn.rnn_cell.GRUCell(num_units=self.hparams.num_units // 2, reuse=tf.AUTO_REUSE, name=f'{self.name}_bw/', dtype=self.hparams.dtype)
    ...
    def decoder(self, _input_states, _last_state, _input_sequence_length, mode, _encoder_inputs=None, sampling_probability=None):
        ...
        with tf.variable_scope(f'{self.name}_basic_decoder', reuse=tf.AUTO_REUSE):
            decoder_initial_state = self.decoder_cell.zero_state(self.batch_size, dtype=self.hparams.dtype).clone(
            ...
            if mode == 'denoising' or mode == 'backtranslation_main':
                ...
                projection_layer = tf.layers.Dense(self.hparams.vocab_size, use_bias=False, dtype=self.hparams.dtype)
    ...
    def loss(self, _prevent_positional=None,
        ...
        if lan1_meaning is not None and lan2_meaning is not None:
            ...
            meaning_se = tf.zeros(1, dtype=self.hparams.dtype)
        
        train_loss = tf.cast(tf.reduce_sum(cross_entropy * target_weights), dtype=self.hparams.dtype) + tf.cast(tf.reduce_sum(meaning_se), dtype=self.hparams.dtype)      
        return train_loss / tf.cast(tf.to_float(tf.shape(labels)[0]), dtype=self.hparams.dtype)
    ...
    def backtranslation_model(self, _input, _input_sequence_length, sampling_probability, lan_enc_dec, translation=False):
